function [KSt,TSt,xRSt] = calc_StabLim_KTz2Tanh_xR_Adiabatic(par,K_values,T_values,use_complex_ev)
    % par has to define:
    ...
    %   Z -> (=Q) the selected value of the external field for V
    %   xR, K, T don't matter, since K,T will be replaced by K_values,T_values, respectively, and xR are calculated inside this function
    ...
    for i = 1:numel(KSt)
        ...
        xRSt{i}                 = clean_data(Qfast,[],[],xR_StLim(par.l,par.d,par.Q,xSSt{i},Qfast));
...
function l = lambda_StLim(delta,xR,xS,Q)
    l = delta.*Q./(xR-xS);
end

function d = delta_StLim(lambda,xR,xS,Q)
    d = lambda.*(xR-xS)./Q;
end
