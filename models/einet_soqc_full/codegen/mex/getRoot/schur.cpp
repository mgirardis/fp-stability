...
static emlrtRTEInfo nb_emlrtRTEI = { 15,/* lineNo */
  5,                                   /* colNo */
  "xgehrd",                            /* fName */
...
static emlrtRTEInfo qb_emlrtRTEI = { 100,/* lineNo */
  9,                                   /* colNo */
  "xhseqr",                            /* fName */
  "C:\\Program Files\\Polyspace\\R2019b\\toolbox\\eml\\eml\\+coder\\+internal\\+lapack\\xhseqr.m"/* pName */
};
...
void schur(const emlrtStack *sp, emxArray_creal_T *A)
{
  ...
  if (anyNonFinite(&st, A)) {
    ...
    A->size[0] = m;
    A->size[1] = istart;
    emxEnsureCapacity_creal_T(sp, A, j, &mb_emlrtRTEI);
    for (j = 0; j < istart; j++) {
      for (i = 0; i < m; i++) {
    ...
    emxInit_creal_T(sp, &tau, 1, &pb_emlrtRTEI, true);
    st.site = &ab_emlrtRSI;
    b_st.site = &fb_emlrtRSI;
    j = tau->size[0];
    tau->size[0] = A->size[0] - 1;
    emxEnsureCapacity_creal_T(&b_st, tau, j, &nb_emlrtRTEI);
    if (A->size[0] > 1) {
    ...
    emxFree_creal_T(&tau);
    emxInit_creal_T(&b_st, &w, 2, &qb_emlrtRTEI, true);
    st.site = &bb_emlrtRSI;
    ...
    w->size[0] = 1;
    w->size[1] = A->size[0];
    emxEnsureCapacity_creal_T(&b_st, w, j, &ob_emlrtRTEI);
