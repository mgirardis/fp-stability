function fp = fixedPointGLNetEISOqCFull(par, s, r)
...
function [rhoS,WS,thetaS] = fixedPointGLNetEISOqC_internal(par, s, r)
    ...
    if (s < 0) && (r < 0) % the solution with theta > 0
        rhoS = 1./(par.tauT .* par.u) + zeros(sz);
        WS = par.A./(1+par.tauW./par.tauT) + zeros(sz);
        ...
        rhoS = (par.A-WS)./(par.u.*WS.*par.tauW);
...
function c = calc_W_poly_coeff(par,sz)
    c1 = zeros(sz) + par.q.*par.G.*(1+par.u.*par.tauW);
    c2 = zeros(sz) + (-1).*par.J.*par.p.*par.G.*(1+par.u.*par.tauW)+(-1).*par.A.*par.q.*par.G.*(2+par.u.*par.tauW)+par.u.*par.tauW.*(1+par.G.*par.I+par.u.*par.G.*par.I.*par.tauW);
    c3 = zeros(sz) + par.A.*(par.A.*par.q.*par.G+(-1).*par.u.*(1+par.G.*par.I).*par.tauW+par.J.*par.p.*par.G.*(2+par.u.*par.tauW));
